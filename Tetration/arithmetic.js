...
,GreaterQ = function(x,y){return LessQ(y,x)}
,LessEqualQ = function(x,y){return !GreaterQ(x,y)}
,GreaterEqualQ = function(x,y){return !LessQ(x,y)}
,Min = function(x,y){return LessQ(x,y)?x:y}
,Max = function(x,y){return LessQ(x,y)?y:x}
,Exp = function(x){//If input is normalized, it does not change Object into Number
	var abx;
...
,Ln = function(x){return typeof x=='object' ? x.neg?NaN:Normal({neg:x.recip,recip:false,val:x.val,pt:x.pt-1}) : Normal(Math.log(x))}
,Abs = function(x){return typeof x=='object' ? ({neg:false,recip:x.recip,val:x.val,pt:x.pt}) : Math.abs(x)}
,Neg = function(x){return typeof x=='object' ? ({neg:!(x.neg),recip:x.recip,val:x.val,pt:x.pt}) : -x}
,Sign = function(x){return typeof x=='object' ? x.neg?-1:1 : Math.sign(x)}//Do not return object
,LessAbsQ = function(x,y){
	var bac;
	if(typeof x=='object'){
		if(typeof y=='object'){
...
,Minus = function(x,y){return Plus(x,Neg(y))}
,Recip = function(x){return typeof x=='object'?({neg:x.neg,recip:!(x.recip),val:x.val,pt:x.pt}):1/x}
,Times = function(x,y){
	var res;
	if(x===0||y===0) return 0;
...
,Divide = function(x,y){return Times(x,Recip(y))}
,Power = function(x,y){
	var res;
...
,Root = function(x,y){return Power(x,Recip(y))}
,Log = function(x,y){return Times(Ln(y),Recip(Ln(x)))}
,Natural = function(x){return Sign(x)<0||x.recip?0:Number.isFinite(x)?Math.round(x):x}
//Library of googological functions, mainly on natural numbers
,IteratedPower = function(base,height,tailend){//base^base^...base^tailend with height base's
...
,Tetrate = function(a,b){return IteratedPower(a,b,1)}
,Factorial = (function(){
	var a=[1],temp;
